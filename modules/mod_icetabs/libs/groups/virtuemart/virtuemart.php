	...
	class LofSliderGroupVirtuemart extends LofSliderGroupBase{
      ...
      $descriptionMaxChars = $params->get( 'description_max_chars', 100 );
          ...
          $rows[$key]->introtext = $rows[$key]->product_desc ;
					...
					if( $rows[$key]->product_image_url&&$image=self::renderThumb( $rows[$key]->product_image_url, $imageWidth, $imageHeight, $rows[$key]->title, $isThumb ) ){
						$rows[$key]->mainImage = $image;
					}
					if( $rows[$key]->product_image_url&&$image=self::renderThumb( $rows[$key]->product_image_url, $thumbWidth, $thumbHeight, $rows[$key]->title, $isThumb ) ){
