...
HTML_flexicontact::displayForm($configData,$formData,$errorMessages,$validation_result);
...
// build the message body from the configured fields
	...
	if (!empty($formData['list1']))
		...
		$body .= $configData['list_prompt'].": ".$formData['list1']."\r\n";
...
// from version 2.15 we build the mail object ourselves so that we can get at the ErrorInfo
	...
	//$subject="Formulario consultas sugerencias";
	...
	logText(JText::_('COM_FLEXICONTACT_SUBJECT').": $subject\r\n".
		"To: $recipient\r\n".
...
// if the user wanted a copy, send that separately
	...
	if ($formData['copyMe'] == 1)
		...
		if ($ret_copy === true)
			...
			$status = "Mail was NOT accepted for delivery (".$mail->ErrorInfo.")\r\n";
		//logText("Copy tol: $recipient\r\n".
		logText("Copy to: ".$formData['fromAddress']."\r\n".
			$status.
			"-------------------------\r\n");
		}
	return $ret_main;
